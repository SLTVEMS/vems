...
const SummaryRowWrap = styled(Box)`
  display: grid;
...
function buildSummaryRows(request) {
  return [
    ...
    { icon: "person", label: "Visitor Name", value: formatValue(request.visitor) },
...
function RejectConfirmationModal({ open, request, onClose, onConfirm }) {
  const summaryRows = useMemo(() => buildSummaryRows(request || {}), [request]);
